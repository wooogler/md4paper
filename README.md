...
본문을 **드래그하면 색을 고르는 막대**가 떠서 하이라이트를 긋고, `메모 달기`로 메모를 붙일 수
있습니다. 표시 단위는 **문장**이라, 몇 글자만 끌어도 그 문장 전체가 잡히고 **원문과 번역 양쪽에
같은 색으로 함께 표시**됩니다 — 메모도 하나로 공유합니다. 하이라이트를 다시 누르면 원문·번역
두 문장이 함께 뜨는 카드에서 색을 바꾸거나 메모를 고치거나 지울 수 있고, 툴바의 메모 아이콘으로
전체 목록을 열어 그 자리로 이동할 수 있습니다. 표시한 내용은 작업 폴더의 `annotations.json`에
저장돼 다음에 열어도 그대로 있고, **메모 md** 버튼으로 원문·번역 인용구와 메모를 마크다운
한 장으로 뽑을 수 있습니다.
