...
문단 위에 마우스를 올리면 **원문과 번역의 짝이 되는 문단이 함께 옅게 표시**됩니다 — 어느 문장이
어느 문장인지 눈으로 따라가기 위한 것입니다.

본문을 **드래그하면 색을 고르는 막대**가 떠서 하이라이트를 긋고, `메모 달기`로 메모를 붙일 수
있습니다. 하이라이트를 다시 누르면 색을 바꾸거나 메모를 고치거나 지울 수 있고, 툴바의 메모
아이콘으로 전체 목록을 열어 그 자리로 이동할 수 있습니다. 표시한 내용은 작업 폴더의
`annotations.json`에 저장돼 다음에 열어도 그대로 있고, **메모 md** 버튼으로 인용구+메모를
마크다운 한 장으로 뽑을 수 있습니다.
