...
- **설치 방식이 바뀌면 작업 폴더가 따라 움직였다** — 기본값이 실행 위치에 달려 있어(클론:
  `<저장소>/output`, 설치본: `~/md4paper/output`), 클론으로 변환해 둔 사람이 한 줄 설치로 갈아타면
  목록이 빈 폴더를 가리켜 '논문이 사라진' 것처럼 보인다(실제로 겪었다). UI가 처음 뜰 때
  `config.pin_workspace()`로 그때의 기본값을 config에 적어 고정하고, 목록이 비면 지금 보고 있는
  폴더 경로와 바꾸는 길을 함께 보여준다. 배포 경로를 둘로 만든 이상 이건 흔한 사고가 된다.

---

## 14. 프로젝트 — 논문을 묶어 관리 + BibTeX 쌓기 (구현 완료)

### 목표
한 사람이 동시에 여러 주제를 읽는다(학위 서베이, 수업 과제, 진행 중 연구). 그런데 저장 위치가 **전역 하나**라
모든 논문이 한 폴더에 섞이고, 홈 목록도 최근순 한 줄로만 쌓였다. **프로젝트**는 그 둘을 함께 푼다:
논문을 묶어 걸러 보고, 묶음마다 **폴더 하나**를 정하면 그 안이 자동으로 정리된다. 같은 폴더에
`references.bib`이 쌓여 인용할 때 그대로 복사해 붙인다.

### 설계
1. **폴더는 프로젝트마다 하나, 자리는 그 안에서 잡는다** — en/ko/pdf 세 칸을 프로젝트마다 또 받으면 설정이
   세 배가 된다. 루트 하나만 받고 `projects.layout()`이 자리를 정한다: 영어는 **루트**(노트 앱에서 폴더를 열면
   논문 목록이 바로 보이는 게 자연스럽다), 번역은 `ko/`, 원본은 `pdf/`, 서지는 `references.bib`.
   `set_root`가 그 자리를 미리 만들어 둔다(고르자마자 구조가 보이게).
2. **폴백은 공통 저장 위치** — 폴더를 안 정한 프로젝트와 미분류 논문은 기존 `[library].en_dir/ko_dir/pdf_dir`로
   간다. 프로젝트를 쓰기 전에 잡아 둔 설정이 그대로 살아 있고, 기존 테스트도 그대로 통과한다.
   해석은 `library.dir_for(which, project)` 한 곳에 모았다(프로젝트 폴더 > 공통 > None).
3. **배정은 논문 쪽에 적는다** — 프로젝트 id를 그 논문의 `status.json["project"]`에 둔다(`hidden`·`pinned_at`과
   같은 자리). 목록을 훑을 때 한 번에 읽히고, `projects.json`이 사라져도 논문이 사라지지 않는다.
   반대로 두면(프로젝트가 논문 경로 목록을 갖는 방식) 리네임·삭제마다 두 파일을 맞춰야 한다.
4. **이름이 아니라 id로 묶는다** — 이름을 바꿔도 배정이 따라온다. 없어진 id는 어디서나 미분류로 읽어
   (`projects.normalize`) 유령 구역이 생기지 않는다 — 그래서 프로젝트 삭제가 논문을 건드리지 않아도 된다.
5. **옮기면 사본도 따라간다** — `library.reassign`이 옛 프로젝트 폴더의 사본(마크다운·이미지·PDF·bib 항목)을
   지운 뒤 새 자리로 내보낸다. 안 그러면 옛 볼트에 고아가 남고, 원본을 지운 뒤엔 그게 어디 있었는지 알 수 없다.
   사용자가 **직접 누른 동작**이므로 자동 저장이 꺼져 있어도 옮긴다(이름 정리와 같은 규칙).
   삭제는 `remove_stem_everywhere`로 공통 + 모든 프로젝트 폴더를 훑는다(옮겨 다닌 흔적까지).
6. **고른 프로젝트가 새 논문의 소속** — 목록 필터와 '새 논문이 들어갈 곳'을 하나로 뒀다(`projects.active()`).
   보고 있는 묶음에 올린 논문이 그 묶음에 들어가는 게 기대에 맞고, 설정이 하나 줄어든다. 드롭존이 어디로
   들어갈지 문장으로 밝혀 조용한 배정이 되지 않게 했다.
7. **BibTeX는 만들지 않고 검증한다** — 논문 제목에는 `&`, `%`, `_`, 악센트가 흔하다. 이스케이프를 직접 짜면
   언젠가 `\cite`가 아니라 **LaTeX 컴파일이** 깨지고, 그걸 마감에 알게 된다. 그래서 이스케이프·직렬화를
   **bibtexparser**(MIT, 순수 파이썬)에 맡기고, 만든 항목을 **되읽어**(`verify`) 키·종류·필드가 그대로
   돌아오는지 본 뒤 파일에 쓴다. 쓴 다음 파일 전체를 다시 파싱해 항목이 줄었으면 **쓰기 전으로 되돌린다**.
8. **파일 전체를 재직렬화하지 않는다** — 손으로 넣은 항목·주석은 사용자의 것이다. 우리 키의 블록만 제자리에서
   갈아치우고(브레이스 카운팅으로 범위를 찾고, 7의 사후 파싱이 안전망), 없으면 뒤에 덧붙인다. 그래서 같은
   논문을 다시 내보내도 항목이 늘지 않고 **그 자리에서 최신이 된다**(서지 보강을 돌린 뒤가 특히 그렇다).
9. **인용 키 = 논문 기준명** — 마크다운·PDF 파일명과 같은 이름이라 노트의 `[[2017_Attention_Vaswani]]`와
   `\cite{2017_Attention_Vaswani}`가 어긋나지 않는다(BibTeX 키에 못 쓰는 문자만 걷어낸다).

### 파일
- `projects.py`(신규): `layout`/`ensure_layout`/`all_projects`/`get`/`exists`/`normalize`/`name_of`/`create`/
  `rename`/`delete`/`root_of`/`set_root`/`dir_for`/`has_dirs`/`active`/`set_active`/`assign_target`/`options`.
  저장은 `~/.config/md4paper/projects.json`(원자적 쓰기) — config.toml에 안 넣은 이유는 우리 최소 TOML
  직렬화기가 테이블 배열을 못 쓰기 때문이다.
- `bibtex.py`(신규): `cite_key`/`entry_kind`/`fields_from_meta`/`render`/`parse`/`verify`/`entry_for`/
  `sync`/`remove`/`keys_in`. 의존성 `bibtexparser>=1.4` 추가.
- `library.py`: 모든 해석에 `project` 인자(`dir_for`/`configured`/`auto_enabled`/`same_folder`/`file_name`),
  `project_of`/`dirs_for`/`bib_path`/`export_bib`/`remove_stem_everywhere`/`reassign` 신규.
- `workdir.py`: `set_project`(mtime 되돌림 — 배정은 '작업'이 아니다)/`project_of`, `recent_workdirs`에 `project`,
  `delete_workdir`이 `remove_stem_everywhere`를 부른다.
- `config.py`: `resolve_library_bibtex()`(`[library].bibtex`, 기본 켜짐).
- `paper_meta.py`: 이름 정리 시 옛 사본을 **그 논문의 프로젝트 폴더**에서 지운다.
- `ui/app.py`: `build_project_settings` 패널(만들기·이름·폴더 하나·삭제), 목록의 프로젝트 필터·구역 묶기·
  카드 프로젝트 칩(옮기기)·다중선택 일괄 이동, 큐 항목에 소속을 실어 변환 완료 시 배정,
  저장 위치 패널의 '폴더 하나로 정리'·BibTeX 스위치.
- `cli.py`: `md4paper project`(list/add/rm/dir/use/assign/export), `library --root/--bibtex`, `md4paper bib`.
- 테스트: `test_projects.py`(CRUD·layout·폴백·배정·reassign·삭제 정리), `test_bibtex.py`(항목 종류·이스케이프·
  덧붙이기/제자리 교체·남의 항목 보존·검증 실패 되돌리기).

### 리스크·결정
- **프로젝트 삭제가 논문을 고아로 만들지 않는가** — 만들지 않는다. 없는 id는 미분류로 읽히고(4), UI는 삭제할 때
  그 논문들의 배정까지 지운다. 폴더·파일은 건드리지 않는다 — 사용자의 볼트다.
- **폴더를 다른 프로젝트와 같게 정하면** 두 묶음의 논문이 한 폴더에 섞인다. 막지 않았다 — 파일명이 기준명이라
  충돌하지 않고, "EN/KO를 한 폴더로" 같은 의도적 사용을 금지할 이유가 없다.
- **`en`이 루트라 루트가 어수선해진다** — 논문 수가 수백 편이면 그렇다. 그래도 노트 앱에서 볼 때 한 단계
  덜 들어가는 값이 크다고 봤다. 하위 폴더로 넣고 싶으면 공통 저장 위치에서 세 칸을 직접 지정하면 된다.
- **`ko/`에도 이미지가 복사된다** — 마크다운이 자기 폴더 기준 `images/…`를 참조하므로 en·ko가 각자 갖는다.
  기존 동작(폴더를 따로 지정했을 때)과 같고, 상대 경로 `../images/…`로 바꾸면 노트 앱마다 해석이 갈린다.
- **.bib이 사용자 파일과 겹칠 때** — 이미 쓰던 `references.bib`이 있으면 우리 항목만 덧붙는다(8). 우리 키와
  같은 키를 손으로 넣어 뒀다면 그 항목은 갈아치워진다 — 키가 기준명이라 사실상 같은 논문일 때만 벌어진다.
- **서지 정보가 없으면 항목을 안 만든다** — 지어낸 저자·연도가 참고문헌 목록에 섞이는 것보다 빈 게 낫다.
  `md4paper enrich --all`로 채운 뒤 `md4paper bib`로 다시 모으면 된다.

### 서지 원본 `bibsource.py` — 논문 API에서 **정확한 BibTeX를 받아온다** (구현 완료)

조립한 항목의 한계가 실측으로 드러났다. NIRVANA 프로젝트 `references.bib`(57항목) 감사:
venue 없음 30 · venue가 논문 각주의 약칭(`CHI '24`) 7(그중 5는 `@article`로 오분류) ·
**DOI·pages·volume·publisher 0** · 연도 오류 6 · 추출 손상(`HaLLMark Efect`) 2 · 같은 논문 중복 1쌍.
PDF 1페이지에 없는 것은 LLM도 못 만든다 → **조립 대신 출판 기록을 받아온다.**

**설계의 핵심은 신원 확인과 서지 원본을 나눈 것이다.**
1. **신원**: Ai2 **Semantic Scholar** `search/match`가 제목으로 논문을 찾는다(실측 63편 중 51편을 한 번에,
   추출 오타 `Efect`까지 유사도 0.996으로). 못 찾으면 Crossref 제목 검색 → OpenAlex.
2. **서지 원본**: 찾은 DOI로 **Crossref**에서 출판사 등록 항목을 그대로 받는다.

나눈 이유: **S2의 `year`는 프리프린트 해다.** 실측 — Crossref 기록이 있는 48편 중 **10편(21%)에서 연도가
어긋났고 어긋날 때마다 Crossref가 맞았다**(CHI '26 논문을 2025로, HaLLMark를 2023으로). S2는 찾는 데만 쓴다.

**게이트를 유사도 0.90 하나로 두지 않는다** — `enrich`의 규칙으로는 부족하다는 걸 실측했다:
- `"…Grammarly to Improve Students' Writing Skills"`(2020)와 `"…Saudi Students' …"`(2024, 다른 논문)의
  유사도가 **0.9595** — 단어 하나 차이로 통과한다. → 저자 성·연도로 교차 확인(`matches`).
- 반대로 출판사 기록에 부제가 없으면 유사도 **0.5606**으로 맞는 논문이 거절된다(Leijten 2013).
  → 한쪽이 다른 쪽의 앞부분이고 저자가 겹치면 채택.
- Crossref는 **1위가 오답인 사례**가 있어 후보를 12개 받아 고른다(`enrich`의 `rows=1`은 이 함정에
  그대로 걸린다 — 위 Grammarly 사례에서 1위가 오답, 2위가 정답이었다).
- 유사도가 같으면 **PDF에서 읽은 연도에 가까운 쪽**을 고른다 — **재수록본** 때문이다. Emig의
  "Writing as a Mode of Learning"(1977, CCC)은 Crossref 1·2위가 2020·2024년 선집 재수록본이고
  원본은 **8위**다. 연도가 멀다고 버리지는 않는다 — 후보가 하나뿐이면 그게 뽑히고, 그래서
  PDF의 연도가 틀린 경우(Flower & Hayes 2008→1981)의 교정은 그대로 동작한다.

**DBLP는 검토 후 뺐다**: booktitle에 개최지·날짜가 박히고, 오타 제목에 퍼지 매칭이 없고(`Efect` → 0건),
CS 전용이며, 무엇보다 **S2가 준 DBLP 키가 형제 레코드를 가리킨 실측 사고**가 있었다(CS50 논문이 SIGCSE
본문 대신 Volume 2 초록으로 연결). 출처를 늘리기보다 출판사 기록 하나를 제대로 쓴다.

**네트워크는 조회 단계에만.** 받은 기록은 `<논문>.md4/bib_source.json`에 캐시하고 `bibtex.entry_for`는
저장된 것만 읽는다 → 내보내기는 오프라인에서도 늘 같은 속도이고, 테스트가 네트워크를 타지 않는다.

**부수적으로 고친 이스케이프 버그 2건**(둘 다 실제 .bib에서 발견):
- 곧은 아포스트로피가 `\textquotesingle`로 과잉 변환돼 `CHI '24`가 `CHI ′24`(프라임)로 조판됐다.
- 굽은 따옴표 `’`(U+2019)는 bibtexparser 표에 없어 **비ASCII 그대로** .bib에 남았다.

### 참고문헌 DOI 채우기 `cite/resolve.py` + 온라인 보강 한 갈래로 합치기 (구현 완료)

`cite` 단계가 뽑은 참고문헌은 **원문에 DOI가 인쇄돼 있을 때만** 링크가 된다. 실측(논문 124편,
참고문헌 7,746건): **56%가 DOI도 arXiv 번호도 없어 링크 없이 나간다.**

**S2의 `/paper/{id}/references`가 논문 하나의 참고문헌 전체를 한 번의 호출로 준다** — 62건짜리
목록을 62번 조회하는 것과는 레이트리밋 앞에서 다른 이야기다. 다만 커버리지가 반반이다(실측):
TextGrad(arXiv) 107건 → 90% 매칭, HaLLMark(CHI) 83건 → 83%, 그런데 Cognitive Offloading(2016
Elsevier)과 Davis(1989 MIS Quarterly)는 **0건**. 링크가 아쉬운 쪽이 바로 그 옛날 논문들이라
한 번에 받아 오는 길만 두면 정작 필요할 때 아무 일도 안 일어난다 → 남은 항목은 한 건씩 찾는다.
한 건씩 찾을 때는 **S2를 빼고**(`BULK_SOURCES`) Crossref부터 간다 — 익명 풀이 초당 몇 건에서도
429를 내서 100건짜리 목록이면 대기만 몇 분이 된다.

**참고문헌은 채우기만 하고 고치지 않는다.** 논문 자기 서지(`bibsource`)는 출판사 기록이 이기지만,
참고문헌에 적힌 건 **저자가 그렇게 인용했다는 사실**이라 제목·연도를 갈아치우면 원문과 어긋난다.

**목록을 다시 렌더할 때 `cite.apply_links`를 쓰지 않는다.** 그쪽은 참고문헌 구역을
`sections.map.json`의 줄 번호로 찾는데 그 번호는 `cite` 단계가 목록을 갈아치운 뒤 어긋나 있다 —
실측: 그대로 불렀더니 구역 끝을 목록 중간으로 잡아 **참고문헌이 109개에서 159개로 늘었다**(중복 50건).
그래서 이미 렌더된 `<a id="ref-N">` 줄의 처음과 끝을 찾아 그 사이만 바꾼다(`relist`). 본문은
손대지 않는다 — 본문 인용은 `#ref-N` 앵커를 가리키고 앵커는 그대로다.

**세 갈래를 한 갈래로.** 온라인 조회가 세 군데로 흩어져 있었다(빈 연도·venue 채우기 / 정확한
BibTeX 받기 / 참고문헌 DOI 채우기). 사용자에게는 전부 "서지 정보를 온라인에서 보강한다" 하나이고
셋 다 같은 API를 같은 순서로 두드리므로 `enrich.boost_workdir` 하나로 묶었다 — 나눠 돌리면
레이트리밋에 세 번 부딪힐 뿐이다. UI 버튼도 **'서지 정보 보강' 하나**로 줄이고,
`md4paper bib`는 **네트워크를 쓰지 않는 조립 전용**으로 되돌렸다.

실측(TextGrad 109건): 링크 14 → **95개**(12%→87%), 45초. 두 번째 실행은 마크다운이 그대로다.
Risko(S2가 0건 준 논문, 101건): 3 → 39개, 한 건씩 찾는 길로만. 한도(`--max-lookups`)를 넘긴
58건은 **조용히 자르지 않고 몇 건인지 알린다.**

### 파일
- `cite/resolve.py`(신규): `fill`/`update_workdir`/`relist`/`_bulk`. 한도 기본 60건.
- `enrich.py`: `boost_workdir`(세 단계를 한 번에)/`enrich_many`가 그걸 부른다.
- `bibsource.py`(신규): `matches`/`surnames`/`resolve`/`fetch`/`normalize`/`arxiv_id`/
  `load`/`update_workdir`/`update_many`. 429에 `Retry-After`가 없고 **정상 질의에도 500이 오는**
  S2의 실측 특성 때문에 `_get`이 429·5xx를 자체 백오프(4s→10s)로 재시도한다.
- `bibtex.py`: `fields_from_meta`가 출판 기록(`kind`·pages·volume·publisher·series·month·editor…)을
  받아 적고, `author_latex`가 있으면 저자 문자열을 **다시 접지 않는다**(`van der Berg, Jan`이
  `Berg, Jan van der`로 바뀌는 것을 막는다). `_escape`에 `_TYPO`·`_UNQUOTE_RE` 추가.
- `enrich.py`: `usable_title(min_words=)` — 두 단어짜리 진짜 제목("Cognitive Offloading")이
  조회조차 안 되던 것을 `bibsource`만 2로 낮춰 쓴다(교차 확인이 있으므로).
- `workdir.py`: `bib_source_json`(`<root>/bib_source.json`).
- `cli.py`: `md4paper enrich`가 세 단계를 다 한다(`--max-lookups`). `md4paper bib`는 오프라인 조립 전용.
- `ui/app.py`: '서지 정보 보강' 버튼 하나가 세 단계를 다 한다(따로 만들었던 BibTeX 버튼은 없앴다).
- 테스트: `test_bibsource.py`(게이트·사다리·출판 기록 우선·재수록본·철회·캐시), `test_cite_resolve.py`(한 번에/한 건씩·빈 칸만 채움·한도 보고·목록 중복 방지).

### 리스크·결정
- **틀린 서지가 빈 서지보다 나쁘다** — 게이트를 통과하지 못하면 아무것도 채택하지 않고 PDF에서 읽은 값을
  그대로 둔다. 실측에서 Dale & Chall 1948은 어느 API에도 없어 그대로 남았다(정상 동작).
- **출판사 기록이 PDF를 덮어쓴다** — `enrich`(빈 칸만 채움)와 반대 방향이다. BibTeX 항목에 한해서만
  그렇고 `paper_meta.json`은 건드리지 않는다 — 목록·검색·파일명은 PDF에서 읽은 값을 계속 쓴다.
- **API 키가 없다** — S2 익명 풀은 초당 몇 건에서도 429를 낸다. 한 편에 1초 간격 + 재시도로 견딘다.
  수백 편을 한 번에 돌리면 오래 걸린다(63편에 약 2분).
- **제목이 심하게 깨진 논문은 못 찾는다** — 추출이 제목을 통째로 놓치면 조회 자체가 막힌다(`usable_title`).
  그건 추출 단계에서 고칠 일이지 여기서 추측할 일이 아니다.

### 변환 파이프라인에 서지 보강 붙이기 (구현 완료)

논문을 올린 **그 시점**에 `references.bib`이 정확해야 한다. 예전에는 변환이 끝나면 PDF에서 읽은
값으로 항목이 쌓이고, 정확해지는 건 사용자가 나중에 '서지 정보 보강'을 따로 돌릴 때였다 —
그런 게 필요한 줄 알 방법이 없다.

변환 큐에 단계 하나(`_auto_bibsource`)를 넣었다. **놓이는 자리가 이 변경의 전부다**:

```
추출 → ② 참고문헌 → ③ 용어집 → ④ 서지 추출 → 이름 정리 → 프로젝트 배정
                                          → ⑤ 서지 보강(온라인) → 저장 위치로 내보내기
```

- **`library.auto_export` 앞**이라야 그때 쓰이는 .bib 항목이 정확하다. 뒤에 두면 처음 쌓이는
  항목은 여전히 부실하고, 다음 내보내기까지 그대로 남는다.
- **`set_project` 뒤**라야 그 프로젝트의 `bib_lookup` 설정을 볼 수 있다.
- **`_auto_rename` 뒤**라야 인용 키(폴더 기준명)가 최종 이름이다.

`[library].bib_lookup`(기본 켬, 프로젝트별로 덮어쓸 수 있음)으로 끈다. 기본을 켠 이유는 변환이
이미 몇 분 걸리는 작업이라 십몇 초가 체감되지 않는 반면, 꺼 두면 .bib이 부실한 걸 알아채기
어렵기 때문이다. 실패해도 조용히 넘어간다 — 네트워크가 없어도 변환은 되어야 한다.

**저자 이름은 판정을 좁혔다.** 처음엔 이니셜 개수를 세어 '출판사가 줄였으면 PDF 표기를 쓴다'로
했는데, 가운데 이름 이니셜을 줄임으로 잘못 읽었다 — `Cecilia D. Shelton`은 `Cecilia Shelton`보다
**더** 자세한데도 PDF 쪽으로 되돌려, 출판사가 바로잡아 준 오타(`Tasfa`→`Tasfia`)를 되살렸다.
이제 **첫 이름 토큰만** 본다(`Wu, Y.` vs `Wu, Yi-Fang Brook`). 실측: 이 판정으로 NIRVANA 74항목 중
9항목의 온전한 이름이 살고, 출판사가 고쳐 준 2항목은 정본을 유지한다.

### 보강 속도 — 병렬 처리와 레이트리밋 (구현 완료)

처음엔 참고문헌을 **한 건씩 직렬로** 찾았다. 순수 IO(HTTP 대기)라 대기 시간이 그대로 더해져
논문 한 편(참고문헌 14건)에 26.7초가 걸렸다. 5개씩 겹쳐 던지도록 바꿨다(`cite.resolve.WORKERS`).

**그런데 병렬로 바꾸자 오히려 나빠졌다.** 세 가지가 연달아 드러났다:

1. **Crossref polite pool을 안 쓰고 있었다.** User-Agent에도 파라미터에도 연락처가 없어 엄격한
   공개 풀에 있었고, 동시 요청 5개에 바로 429가 났다. `mailto`를 실어 해결.
2. **429 한 번에 출처 하나를 통째로 쉬게 했다.** 잠깐 몰린 것뿐인데 Crossref가 2분간 막히고,
   그 뒤 논문들이 **0.0초에 아무 일도 안 하고** 지나갔다 — 빠른데 하는 일이 없으면 느린 것만
   못하다. 이제 **연속 3회**(`STRIKES`) 만에 쉬게 하고, 한 번 성공하면 카운터가 0으로 돌아간다.
3. **백오프가 옛 설계에 맞춰져 있었다.** `(4, 10)`은 S2가 **유일한** 신원 해결기이던 시절 값이다.
   출처가 셋이고 회로차단기가 생긴 지금은, 스레드마다 14초씩 태우고서야 차단되는 손해만 남는다
   → `(2,)`로 줄였다.

그리고 429에도 두 종류가 있어 쉬는 시간을 나눴다 — **예산 소진**(OpenAlex가 종량제로 바뀌었다:
"Insufficient budget … Resets at midnight UTC")은 오늘 안에 안 풀리니 길게(900초), 단순 과속은
짧게(20초). 이 구분이 없을 때 실패 한 건마다 14.6초를 태웠다.

실측(NIRVANA 74편): 편당 약 15초, 전체 약 18분. 논문 **사이**는 병렬로 돌리지 않았다 — 병목이
우리 동시성이 아니라 API 레이트리밋이라, 더 겹쳐 봐야 차단기만 더 자주 걸린다.

**저자 이름은 PDF 쪽이 나을 때가 있다.** 출판사가 이니셜로 등록해 둔 경우가 있어(실측: 73항목 중
10항목이 `Wu, Yi-Fang Brook` → `Wu, Y.`) `bibsource.prefer_fuller_authors`가 **같은 사람들일 때만**
(성 집합이 같을 때) PDF 표기를 되살린다. 저자 구성이 다르면 출판사 기록을 믿는다.

### 프로젝트별 설정 + 보강 범위·진행률·중지 (구현 완료)

**프로젝트마다 다른 설정.** 프로젝트는 이미 '폴더 하나'가 정체성이라, 그 폴더에 무엇을 어떤
이름으로 쌓을지가 묶음마다 달라지는 게 자연스럽다(볼트마다 규칙이 다르다). 그래서 **저장·출력에
관한 넷만** 프로젝트로 내렸다: `naming` · `export_target` · `bibtex` · `auto`.

- **덮어쓰기(override) 모델**이지 복사가 아니다. 프로젝트는 **바꾼 항목만** 저장하고 나머지는
  전역을 그대로 물려받는다 → 전역을 고치면 따로 정하지 않은 프로젝트가 저절로 따라온다.
  설정을 두 군데서 관리하지 않게 하는 게 목적이다.
- `None`(안 정함)과 `False`(꺼짐으로 정함)를 **구별**한다. 안 그러면 프로젝트에서 끈 BibTeX가
  전역 켜짐으로 되살아난다.
- `config.resolve_*(project=None)`가 순서를 정한다: 프로젝트 설정 > config.toml > 내장 기본.
  `projects`가 `config`를 import하므로 반대 방향은 함수 안에서만 한다(`project_setting`).
- 호출측은 **논문이 속한 프로젝트**를 넘긴다(`library.export_bib`/`export`, `paper_meta.folder_base`)
  — 설정 화면이 보는 '지금 고른 프로젝트'와 다르다. 논문마다 갈 곳도 규칙도 달라질 수 있다.
- 추출·번역 설정까지 내리지 않은 이유: 프로젝트 설정 화면이 전역 설정 화면을 통째로 복제하게 되고
  지금 보는 값이 어디서 왔는지 헷갈린다.

**보강 범위를 프로젝트로.** 사람은 한 번에 한 묶음을 손보는데 전체 작업 폴더를 훑으면 지금 쓰는
논문 20편을 고치려다 예전 프로젝트 100편까지 API를 두드린다(실측: NIRVANA 74편 vs 전체 135편).
`enrich.project_roots`가 **홈에서 고른 프로젝트를 그대로 따른다**('모든 프로젝트'면 전체, '미분류'면
미분류). 표식이 하나 필요했다 — `projects.ALL`이 `''`이라 `normalize()`가 미분류에 쓰는 `''`와
구별되지 않아, `enrich.ALL = "*"`를 따로 뒀다.

**진행률과 중지.** 편당 몇 초에서 몇십 초라(참고문헌을 한 건씩 찾으면 100건에 2분) 스피너만으로는
얼마나 남았는지 알 수 없다. `enrich_many(should_stop=...)`가 **논문 경계에서만** 멈춘다 — 한 편의
중간에서 끊으면 반쯤 보강된 논문이 남는다. 멈춰도 그때까지는 이미 파일에 저장돼 있고 `stopped`가
True가 된다. UI는 진행 막대 + '중지' 버튼, CLI는 `click.progressbar` + Ctrl+C.

### 파일
- `projects.py`: `SETTINGS`(넷) · `setting`/`set_setting`/`settings_of`.
- `config.py`: `project_setting()` + `resolve_naming_template`/`resolve_export_target`/
  `resolve_library_bibtex`/`resolve_library_auto`가 `project` 인자를 받는다. `EXPORT_TARGETS` 공개.
- `enrich.py`: `project_roots`/`ALL`, `enrich_many(should_stop=, on_progress=)`에 `total`·`stopped`.
- `library.py`·`paper_meta.py`: 논문의 프로젝트를 resolve에 넘긴다.
- `cli.py`: `md4paper project set <프로젝트> [KEY] [VALUE] [--unset]`, `md4paper enrich --project`.
- `ui/app.py`: 프로젝트 행마다 '이 프로젝트만 다르게' 접이식, 보강 진행 막대 + 중지 버튼.
- 테스트: `test_projects.py`(전역 따름·덮어쓰기·False와 안 정함 구별·해제·잘못된 규칙 폴백·
  삭제된 프로젝트), `test_bibsource.py`(범위 셋·논문 경계 중지).

## 15. 분할 라이브 프리뷰 — 원문 편집기와 프리뷰를 나란히, 스크롤·내용 동기화 (구현 완료)

### 목표
변환 탭의 마크다운 편집이 프리뷰를 **대체**하던 것을 **나란히 서는** 것으로 바꾼다. 왼쪽 원문 편집기,
오른쪽 렌더 프리뷰, 스크롤이 서로 따라오고 타이핑이 바로 반영된다(VS Code의 마크다운 프리뷰와 같은 형태).

### 왜 위지윅이 아닌가 (실측 근거)
위지윅(Milkdown 등)도 검토했고 실제로 코퍼스를 왕복시켜 봤다. 두려워하던 손실 — `<a id="ref-N"></a>`가
사라져 인용 링크가 다 죽는 것 — 은 **일어나지 않았다**(remark·Milkdown 모두 100% 보존). 진짜 비용은 다른 데
있었다.
- **재조판**: 저장할 때마다 파일의 23~28% 줄이 다르게 찍힌다(표 재정렬 2,829행, `\[` 이스케이프 9,609개).
  옵션으로 못 막는다. 오타 하나 고친 diff가 매번 문서 전체가 된다.
- **글자 손상 36행**: CommonMark가 절 번호를 번호목록으로 보고 다시 매긴다 — `2. 2.1 Tool background` →
  `3. 2.1 ...`, `2008)` → `2008.`. 논문 절 번호와 인용 연도가 조용히 틀려진다.
- **표 붕괴**: 첫 저장은 멀쩡하고 두 번째 저장에서 5열 표가 8열로 무너진다(`@milkdown/core`의 커스텀 `text`
  핸들러가 `state.safe()`를 건너뛴다). 82편 중 5편은 저장을 반복해도 수렴하지 않는다.

분할 뷰는 마크다운 원문이 그대로 진실의 원천이라 **파싱→직렬화 왕복이 아예 없다 → 이 비용이 전부 0**이다.

### 설계
1. **줄 번호 정렬** — `anchored_markdown()`이 `sec-<id>`·`data-page` 앵커를 헤더 **앞의 새 줄**로 끼우던 것을
   헤더 **줄 끝**으로 옮겼다. 새 줄로 끼우면 프리뷰 줄 번호가 헤더 개수만큼(최대 261줄) 밀려 싱크가 성립하지
   않는다. 코퍼스 105편에서 헤더 줄 번호가 원문과 100% 일치함을 확인했다.
2. **렌더러** (`ui/mdrender.py`) — 변환 탭 프리뷰를 `ui.markdown`(markdown2)에서 markdown-it-py로 옮겼다.
   블록 토큰의 `token.map`이 소스 줄 범위를 주고, 그것을 `data-line` 속성으로 내보낸다(VS Code와 같은 방식).
   앵커 간격 중앙 4줄·최악 7줄, 렌더 중앙 9.4ms(markdown2는 50.7ms, 최대 461ms).
   덤으로 지금 프로덕션에서 실제로 잘못 보이던 것들이 고쳐진다 — `snake_case_name` → `snake<em>case</em>name`
   (194건/10편), 금액 `$0.08 per Find, $1.41`이 통째로 MathML이 되어 사라지던 것(12편), `<T2>`가 진짜 태그로
   해석돼 본문이 없어지던 것(1편), URL 안 공백으로 죽어 있던 DOI 링크 94개.
3. **레이아웃** — 보기 모드 토글(마크다운 / 마크다운+PDF / PDF)은 손대지 않았다. 편집은 4번째 모드가 아니라
   마크다운 패널의 모디파이어라, 켜면 그 자리가 중첩 splitter로 쪼개진다. 상태 수는 예전과 같은 5가지.
   **동시 표시 패널 ≤ 2** — 편집을 켜면 PDF를 접고 '접힘' 배지로 되돌릴 길을 둔다(3분할은 기본 창에서
   260/227/162px라 못 읽는다). 변환 탭 좌측 설정 열에 접기 버튼과 폭 기억을 붙였다 — 이게 없으면 2분할도
   324px씩이라 못 쓴다(실측 가독 하한은 패널 400px). 접으면 612/611px가 나온다.
4. **스크롤 싱크** (`_md_sync_js`) — `_pdf_sync_js`와 같은 뼈대(dataset 플래그로 1회 바인딩, rAF 스로틀,
   `[data-*]` 훑어 상단 기준 요소 찾기). 에디터 쪽은 DOM이 아니라 CM6 높이맵(`lineBlockAtHeight`)을 읽어
   가상 스크롤에 영향받지 않는다. 앞뒤 앵커 사이를 비율로 보간해 소수 줄을 낸다. 실측 양방향 **±1줄**.
   PDF는 프리뷰 뒤에 체인으로 붙는다(프리뷰를 프로그램적으로 밀어도 scroll 이벤트가 나므로 새 코드가 없다).
5. **라이브 갱신** — 120ms 디바운스 후 바뀐 **블록만** 갈아 끼운다. 전체 재렌더는 274KB를 통째로 내려보내고
   innerHTML을 갈아엎으며 그림을 파괴·재생성해 컨테이너 높이를 무너뜨린다(scrollTop이 0으로 잘리고
   돌아오지 않는다). 블록 패치는 노드를 살려 두므로 브라우저 스크롤 앵커링이 알아서 잡는다.
   실측: 키 입력 → 프리뷰 반영 **198ms**(디바운스 포함), **스크롤 튐 0px**.
6. **수동 편집 보호** — 섹션 트리 레벨 하나를 바꾸면 `change_level → commit → save_and_reassemble →
   clear_manual_edit`으로 편집 내용도, 그것을 알리던 배지도 함께 사라졌다. 분할 뷰는 트리와 에디터를 상시
   동시 노출하므로 이 사고가 상시화된다. 재조립 경로 앞에 `guard_manual_edit()` 확인 다이얼로그를 걸었고,
   취소하면 위젯 값을 되돌린다. `save_en_markdown()`은 tmp+`os.replace`로 원자화했다(status.json과 같은 이유).

### 파일
- `ui/mdrender.py` 신규 — markdown-it-py 렌더러(html=True), `data-line` source-map 플러그인,
  dollarmath(`allow_digits=False` — 금액 오인 24/24 → 3/24), 링크 주소 공백 제거, `blocks()`.
- `ui/app.py` — `anchored_markdown` 인라인 앵커, `preview_pane`/`editor_pane` 분리, `_conv_md_pane` 중첩
  splitter, `_md_sync_js`, `live_update`/`_live_patch`, `guard_manual_edit`, 좌측 열 접기, CSS.
- `ui/controller.py` — `save_en_markdown` 원자적 쓰기.
- `ui/find_bar.py` — `.cm-editor` 서브트리 제외(CM6 가상 스크롤이라 화면 밖 96%를 못 보면서 프리뷰와
  이중 집계된다).
- `ui/scroll_memory.py` — `.cm-scroller` 추가.
- 테스트: `test_mdrender.py`(raw HTML 보존·금액·수식·snake_case·data-line·blocks 등가성),
  `test_ui_controller.py`(앵커가 줄 번호를 밀지 않음, 원자적 쓰기).

### 리스크·결정
- **수식은 버리지 않았다** — 코퍼스의 `$...$` 33건이 전부 금액·JS 템플릿이고 진짜 수식은 0건이지만,
  LLM 수식 인식이 붙으면 진짜 수식이 들어온다. dollarmath + latex2mathml로 **지금보다 나은** 상태로 뒀다.
  남은 오인 3건은 프롬프트 예시(`$WHOLE$`)와 표 헤더(`Input Cost ($)`)라 본질적으로 애매하다.
- **뷰어 탭은 markdown2 그대로** — 하이라이트·메모 앵커가 렌더 HTML이 아니라 `.anno-scope`의 평문
  오프셋이라(annotations.py) 렌더러를 바꾸면 기존 메모가 밀린다. 두 렌더러 공존을 감수했다.
- **자동 저장은 넣지 않았다** — en.md는 `raw.md + sections.yaml`의 파생물이면서 동시에 편집 대상이고,
  다시 만드는 경로가 넷이다. 이 이중 신분을 먼저 정리해야 자동 저장이 안전하다.
- **저장 전 새로고침하면 프리뷰가 마지막 저장 상태로 돌아간다** — 블록 패치는 클라이언트 DOM만 고치고
  서버의 `ui.html` 내용은 낡은 채로 둔다. 에디터도 파일에서 다시 읽으므로 둘은 일관된다.
- **`getElement(id).editor`는 NiceGUI 구현 세부다** — 문서화된 API가 아니라 업그레이드에 깨질 수 있다.
  깨지면 `_md_sync_js`가 조용히 아무것도 안 한다(분할 뷰 자체는 그대로 동작). 줄번호 거터로 읽는 폴백이 있다.
- **pywebview 실물 창에서는 미검증** — 측정은 전부 브라우저 탭이었다. 숨겨진 탭에서 `requestAnimationFrame`이
  멈춰 스로틀이 정지하는 현상이 있는데(다시 보이면 복구된다) 웹뷰가 창 최소화 때 같은지는 확인하지 않았다.
- **GEPA 1편의 코드펜스 짝맞춤** — `` ``` `` 짝이 엉뚱한 곳에서 맞아 본문이 코드블록에 삼켜지고 인용 링크
  10개가 사라진다. 코퍼스 전체에서 홀수 펜스 파일은 0편이라 '누락'이 아니라 규칙 차이다. 렌더러가 아니라
  추출 단계에서 다룰 일이라 별건으로 남겼다.
